...
\begin{itemize}
  ...
  \item {If not previously done, install the packages described in Section~\ref{sec:ScriptInstall}. This can be done by opening the 'installscript.R' file inside the extracted folder with RStudio and clicking 'Source'.}
  ...
  \begin{enumerate}
    \item {Input specifications - these variables affect the report generation as a whole and are constant throughout the entire process. 
    \begin{itemize}
    \item {\textbf{subset\_data} - Enable or disable data subsetting, to compare expected data with real-world data. If enabled, this will }
      \item {\textbf{verbose\_report} - Shows or hides verbose descriptions on PDF report pages.}
      ...
      \item {\textbf{datapath} - Specify the location of the input data file.}
